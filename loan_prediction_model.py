...
d2.isnull().sum()                       #for count of null values in each row
d2.isnull().sum()/d2.shape[0]*100                              #for calculating percentage of null values in each row
d3=d2.drop(["person_gender","person_emp_exp","person_home_ownership","person_education","cb_person_cred_hist_length"],axis=1)
d=pd.DataFrame(d3)
...
sample_data = {
    ...
    'loan_intent': l44,
    'loan_in_rate': l55,
    'loan_percent_income': l66,
    'credit_score': l77,
    'previous_loan_defaults_on_file': l88
}
